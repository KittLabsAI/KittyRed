...
- [Python 3](https://www.python.org/) (开发环境需要；最终用户安装包不需要单独安装)
...
开发模式下，Rust 会继续调用开发机上的 `python3` 或 `PYTHON` 环境变量指定的 Python。
...
发布态会把 `backend/` 服务代码、Python 解释器和 Python 依赖一起打进桌面应用，最终用户下载安装后不需要再单独安装 Python。

### GitHub Actions

- PR / `main` 校验：`.github/workflows/ci.yml`
- 桌面安装包发布：`.github/workflows/release.yml`

更完整的说明见 [docs/github-actions-ci-release.md](./docs/github-actions-ci-release.md)。
